...
const { connectAPI, getInstruments } = require('./connect');

async function init() {
  try {
    const session = await connectAPI();
    const instruments = await getInstruments(session);
    console.log('Successfully connected to FXCM');
  } catch (error) {
